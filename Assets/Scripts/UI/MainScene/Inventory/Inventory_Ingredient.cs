...
public class Inventory_Ingredient : GenericListRectScrolling
{

    public override void Init()
    {
        Debug.Log("재료 아이템 탭 초기화");
        ...
        //할당
        nPlayerTotalItemCount += GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT.Count;
        ...
        {
            //end 조건
            if (i >= GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT.Count)
            {
                nHighItemCount = i - 1;
            ...
            inventoryslots[i].SetItemInfo_Ingredient(GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT[i]);
        }
    ...
    {
        int nRepeatCount = GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT.Count;
        int nRemainder = 0;

        nRepeatCount /= nRowCount;                                                                  //몫                 20
        nRemainder = GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT.Count % nRowCount;        //나머지             0
    ...
    protected override void RePositionSlots(bool _isUp, int _itemIndex)
    {
        ...
        if (_isUp == true)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                //빈칸에 대한 처리.
                if (nItemIndex >= GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT.Count)
                    inventorySlot.gameObject.SetActive(false);
                else
                {
                    //해당 슬롯의 정보 업데이트
                    inventorySlot.SetItemInfo_Ingredient(GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT[nItemIndex]);
                    Debug.Log("ItemIndex : " + nItemIndex + "ItemName : " + (GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT[nItemIndex].strName));
                    nItemIndex++;
                }
    ...
    public override void ContentsListInit()
    {
        ...
        //ListIndex 갱신
        for (int i = 0; i < GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT.Count; i++)
            GameManager.Instance.GetPlayer().LIST_ITEM_INGREDIENT[i].nListIndex = i;
