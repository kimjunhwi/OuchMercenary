...
public class Inventory_Weapon : GenericListRectScrolling
{
    public override void Init()
    {
        Debug.Log("무기 아이템 탭 초기화");
        ...
        //할당
        nPlayerTotalItemCount = GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON.Count;
        ...
        {
            //end 조건
            if (i >= GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON.Count)
            {
                nHighItemCount = i - 1;
            ...
            inventoryslots[i].SetItemInfo_Equipment(GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON[i]);
        }
    ...
    protected override void StartItemScrollingTest(Vector2 value)
    {
        ...
        if (fLastValue > scrollRect.verticalNormalizedPosition)
        {
            ...
            if ((fInitRectYValue + (fSlotSizeX * (1 + nNextIndex)) + 105f) <= fCurHeight && nHighItemCount != GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON.Count)
            {
                //RePosition Slots
    ...
    {
        int nRepeatCount = GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON.Count;
        int nRemainder = 0;

        nRepeatCount /= nRowCount;                                                              //몫                 20
        nRemainder = GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON.Count % nRowCount;      //나머지             0
    ...
    protected override void RePositionSlots(bool _isUp, int _itemIndex)
    {
        ...
        if (_isUp == true)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                //빈칸에 대한 처리.
                if (nItemIndex >= GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON.Count)
                    inventorySlot.gameObject.SetActive(false);
                else
                {
                    //해당 슬롯의 정보 업데이트
                    inventorySlot.SetItemInfo_Equipment(GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON[nItemIndex]);
                    Debug.Log("ItemIndex : " + nItemIndex + "ItemName : " + (GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON[nItemIndex].strName));
                    nItemIndex++;
                }
    ...
    public override void ContentsListInit()
    {
        ...
        //ListIndex 갱신
        for (int i = 0; i < GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON.Count; i++)
            GameManager.Instance.GetPlayer().LIST_ITEM_WEAPON[i].nListIndex = i;
